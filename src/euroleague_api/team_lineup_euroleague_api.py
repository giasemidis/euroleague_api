...
def pbp_data_with_lineups(season, game_code):
    ...
    pbp_data['Indices'] = range(0, len(pbp_data)) # resetting indices since going to loop through them, don't want any duplicates
    pbp_data.set_index('Indices', inplace=True) # resetting indices since going to loop through them, don't want any duplicates
    ...
    on_court_cols = ['Team_A1', 'Team_A2', 'Team_A3', 'Team_A4', 'Team_A5', 'Team_B1', 'Team_B2', 'Team_B3', 'Team_B4', 'Team_B5']
    ...
    players = pbp_data.loc[min(pbp_data.index.tolist()), on_court_cols]

    for idx in pbp_data.index.tolist(): # looping through plays of a game
        if pbp_data.loc[idx, 'PLAYTYPE'] == 'IN':
            sub_time = pbp_data.loc[idx, 'MARKERTIME']
            sub_min = pbp_data.loc[idx, 'MINUTE']
            sub_team = pbp_data.loc[idx, 'TEAM']
            ...
            if pd.isnull(pbp_data.loc[max(all_subs.index), 'Team_A1']): # if I haven't made any replacement yet
                ...
                pbp_data.loc[max(all_subs.index), on_court_cols] = players # replace them in the actualy dataframe
